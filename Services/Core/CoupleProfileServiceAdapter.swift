...
class CoupleProfileServiceAdapter {
    // MARK: - Properties
    ...
    private let coupleProfilesCollection = "coupleProfiles"
    ...
    init(crudService: CRUDService = ServiceManager.shared.getService(CRUDService.self),
         ...
         userService: UserService = ServiceManager.shared.getService(UserService.self)) {
        self.crudService = crudService
        self.relationshipService = relationshipService
        self.userService = userService
        print("📱 CoupleProfileServiceAdapter initialized")
    }
    ...
    func getCurrentCoupleProfile() async throws -> CoupleProfile? {
        ...
        let filter: [String: Any] = [
            "collection": coupleProfilesCollection,
            "relationshipID": relationshipID
        ]
        
        let results = try await crudService.list(filter: filter)
        
        if let profileData = results.first {
            // Found existing profile
            var profile = try FirestoreDecoder().decode(CoupleProfile.self, from: profileData)
            if profile.id == nil, let id = profileData["id"] as? String {
                profile.id = id
            }
            return profile
        }
        ...
        var profileData = try FirestoreEncoder().encode(newProfile) as? [String: Any] ?? [:]
        profileData["collection"] = coupleProfilesCollection
        
        let path = try await crudService.create(profileData)
        ...
        createdProfile.id = components[1]
    ...
    func updateCoupleProfile(_ profile: CoupleProfile) async throws -> CoupleProfile {
        guard let profileID = profile.id else {
            throw ServiceError.invalidOperation("Profile ID is required for updates")
        }
        ...
        let profileData = try FirestoreEncoder().encode(updatedProfile) as? [String: Any] ?? [:]
        try await crudService.update("\(coupleProfilesCollection)/\(profileID)", with: profileData)
    ...
    func updateMetrics(profileID: String, updates: [String: Any]) async throws {
        ...
        try await crudService.update("\(coupleProfilesCollection)/\(profileID)", with: metricsUpdates)
    }
    ...
    func updateAvailabilityPreferences(profileID: String, availabilityPreferences: CoupleAvailabilityPreferences) async throws {
        ...
        // Update the profile
        try await crudService.update("\(coupleProfilesCollection)/\(profileID)", with: [
    ...
    private func getPartnerForRelationship(relationship: Relationship) async throws -> AppUser? {
        ...
        return try await userService.getUser(id: partnerID)
    }
    ...
    private func validateUserCanModifyProfile(profileID: String) async throws {
        ...
        // Get the profile
        let profileData = try await crudService.read("\(coupleProfilesCollection)/\(profileID)")
        
        guard let profileData = profileData else {
            throw ServiceError.notFound("Couple profile not found")
        }
        
        let profile = try FirestoreDecoder().decode(CoupleProfile.self, from: profileData)
        ...
        guard relationship.includesUser(userID: currentUserID) else {
            throw ServiceError.unauthorized("You don't have permission to modify this profile")
        }
    }
} 